...
- `kb/rules/` — rules KB built on top of the above: `fab-cli rules sync` refreshes the vendored CR/TRP/PPG txt (via `rules update-docs`), chunks CR/TRP/PPG by their own numbered-section headings, extracts the vendored Casual Procedure Guide PDF (`docs/references/FaB_Casual_Procedure_Guide_2023-10-13.pdf`) by heading structure, and fetches the live Card Legality Policy page (never cached — re-fetched on every `sync` call) — writing frontmatter markdown chunks (`document`, `section`, `title`, `source_url`, `version`, `fetched_at`) plus a rebuildable `kb/rules/index.json`. `kb/rules/` (chunks + index) is entirely git-ignored and rebuildable via `rules sync` — unlike `lore/`, which commits its OKF `.md` files and only ignores `index.json`/`.sync-state.json`; a re-sync supersedes each document's prior chunk set (no stale chunks) and per-source failures are isolated (one source failing doesn't block the others).
