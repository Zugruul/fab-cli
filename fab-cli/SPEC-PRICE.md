...
- One **price page** per provider: columns `Name, Set, Finish, Code, NM, NM Source, SP/LP, SP/LP Source, MP, MP Source, HP, HP Source`, prices in the provider's native currency.
- One **ratio page** per ordered pair (Pi, Pj), i≠j (v1: tcgplayer/cardmarket and cardmarket/tcgplayer): same identity columns (incl. `Code`); each condition cell is `Pi_price / Pj_price − 1` expressed as a signed percentage (e.g. `+30.0%`), computed **after converting both prices to a common currency** (§8.4). Ratio pages contain only rows that have a price for that condition on **both** providers.
- **Code column**: every page carries a `Code` column, positioned immediately after `Finish` and before the first price column — the official FAB printing identifier for that row (e.g. `EVR141`), resolved against the vendored `third_party/flesh-and-blood-cards` DB by `(canonical card name, canonical set name, finish)`, never guessed and never sourced from either marketplace's own product identifiers. A multi-pitch card (three DB entries, one per pitch, each with its own code) is disambiguated via the marketplace name's parenthesized pitch suffix routed to the DB's `pitch` field; looking it up without a suffix is ambiguous and returns empty rather than guessing. When the vendored DB has no entry matching the exact triple (absent from the DB, or a set-name spelling mismatch between sources) the cell is empty (`—` terminal, `""` CSV) — this is a disambiguation aid, not a match/no-match signal, so an empty Code never causes a row to be treated as unmatched (I4/I7 still apply).
...
- The TCGplayer price table, the Cardmarket price table, and both ratio tables each gain the `Code` column in the position defined in §4.3; the `--csv` output's four pages gain it identically (§9.3).
...
- Price page header (TCGplayer page): `Name,Set,Finish,Code,NM,NM Source,SP/LP,SP/LP Source,MP,MP Source,HP,HP Source`. The **Cardmarket price page header gains a trailing `Trend,Trend Source` companion pair**: `Name,Set,Finish,Code,NM,NM Source,SP/LP,SP/LP Source,MP,MP Source,HP,HP Source,Trend,Trend Source`. Prices are plain decimals (no currency symbol); the currency is stated in a leading comment line (`# currency: USD` / `# currency: EUR`) per page. The Trend column follows the same empty-if-absent rule as every other cell and is never consumed by the ratio pages.
- Ratio page header: `Name,Set,Finish,Code,NM,NM Basis,SP/LP,SP/LP Basis,MP,MP Basis,HP,HP Basis`, preceded by comment lines `# ratio: tcgplayer / cardmarket` and `# fx: 1 EUR = <rate> USD (ECB <date>)`.
- `unmatched.csv` header: `Provider,Name,Set,Finish,Code,Reason`.
- The `Code` cell follows the same empty-if-absent convention as every other CSV field (empty string, never a placeholder) — uniformly across price pages, ratio pages, and `unmatched.csv`.
