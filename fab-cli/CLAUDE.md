...
src/commands/       — Per-namespace Commander registration (one module per namespace)
  ...
  rules.ts          —   rules: update-docs, sync, search, show
...
```bash
fab-cli price-comparison card "<name>"                          # 4-page terminal comparison
fab-cli price-comparison card "<name>" --csv [file]              # emit CSV instead (stdout or file)
fab-cli price-comparison card "<name>" --refresh                 # bypass disk caches, re-fetch live
fab-cli price-comparison card "<name>" --currency usd|eur         # ratio-table common currency (default usd)

fab-cli price-comparison export [--set <name...>] [--out <dir>] [--refresh] [--currency usd|eur]
```

- `card <name>` resolves against the tcgcsv catalog; an ambiguous name prints candidates and
  exits 1. Output is 4 pages: TCGplayer prices (USD), Cardmarket prices (EUR, + reference-only
  `Trend` column), and a ratio table per ordered pair (tcgplayer/cardmarket,
  cardmarket/tcgplayer) converted to a common currency with the dated ECB rate shown above the
  tables.
- `export` runs against the full FAB singles catalog by default (can take a while); `--set`
  filters to one or more tcgcsv group names (case-insensitive, repeatable) — use it for quick
  testing. Writes 5 files to `--out` (default `./price-comparison/`): `prices-tcgplayer.csv`,
  `prices-cardmarket.csv`, `ratio-tcgplayer-cardmarket.csv`, `ratio-cardmarket-tcgplayer.csv`,
  `unmatched.csv`. Prints per-set progress lines and a final summary (rows per page, match
  rate, degraded sets, elapsed time).
- **Real-data-only (HARD RULE, post-issue-#61):** a price cell is either a real observed value
  with a source label (`listing`, `low`, `trend`/`avg30`/`avg7`/`avg1`) or empty (`—`/blank in
  CSV) — there is no adjacency copy, `marketPrice`/`lowPrice` stand-in, or any other fabricated
  fill. Ratio cells only fire when **both** sides have a real cell for that condition, so ratio
  tables are sparser by design.
- **Condition columns are NM, SP/LP, MP, HP, always in that order.** Cardmarket publishes no
  per-condition data, so **all four Cardmarket condition columns use the same price-guide `low`
  value** (source `low`); there's a separate reference-only `Trend` column next to them
  (`trend`, cascading to `avg30`→`avg7`→`avg1` if `trend` is null/absent) that is **never**
  consumed by ratio math.
- **Data sources:**
  - [tcgcsv.com](https://tcgcsv.com) — keyless mirror of the full TCGplayer catalog (groups,
    products, market prices), 24h disk cache. Requires a browser `User-Agent` header (the
    client sets one by default) or requests 401.
  - TCGplayer storefront search (`mp-search-api.tcgplayer.com`) — keyless, gives real live
    per-condition listing prices (the official TCGplayer API is closed to new applicants, so
    this is the only keyless source of per-condition data — there is intentionally no `auth`
    command in this namespace). Not cached (listings are live/volatile); a 403 means
    rate-limiting/bot-protection, not a crash — it degrades to empty condition cells for the
    affected rows, never a stand-in price.
  - Cardmarket S3 downloads (`downloads.s3.cardmarket.com`) — price guide + product catalog
    JSON, 24h disk cache. Only non-Cloudflare Cardmarket source; no per-condition data.
  - [frankfurter.dev](https://frankfurter.dev) — ECB EUR/USD reference rate, 24h disk cache. If
    the FX fetch fails, ratio pages/tables are skipped (exit 1) while price pages/tables still
    render — a real observed price is never blocked by an unrelated FX outage.
- **Cardmarket set-name mapping**: `data/cardmarket-expansions.json` (committed), generated by
  `scripts/cardmarket-expansions.ts` via unique-card-name voting between tcgcsv sets and
  Cardmarket `idExpansion`s, with confidence guards (tie detection + set-size plausibility,
  post-issue-#60). An expansion the voting can't confidently map renders as
  `cm-expansion-<id>` instead of a guessed name — **this is correct, intentional behavior**,
  not a bug to "fix" by loosening the guard.
- **Live TCGplayer listings mean non-determinism**: because `card`'s and `export`'s TCGplayer
  prices come from live storefront listings, two runs against the same card/set minutes apart
  can show slightly different numbers — this is expected. Cardmarket data is bulk-downloaded
  and cached, so it stays stable within the 24h TTL.
- `--refresh` bypasses all three disk caches (tcgcsv, Cardmarket, FX) and re-fetches; use it
  when you need current prices rather than a cached snapshot.
...
- `kb/rules/` — rules KB built on top of the above: `fab-cli rules sync` refreshes the vendored CR/TRP/PPG txt (via `rules update-docs`), chunks CR/TRP/PPG by their own numbered-section headings, extracts the vendored Casual Procedure Guide PDF (`docs/references/FaB_Casual_Procedure_Guide_2023-10-13.pdf`) by heading structure, fetches the live Card Legality Policy page (never cached — re-fetched on every `sync` call), and pulls **Rules Reprise** articles (LSS's per-set interaction-guidance series, e.g. "Rules Reprise: Omens of the Third Age Constructed") from fabtcg.com's WordPress REST API (`GET /api/wp/v2/posts?search=rules+reprise`, paginated, capped at `MAX_REPRISE_ARTICLES`) — one chunk per article (not sub-sectioned), following the same 7-day-TTL refresh model as CR/TRP/PPG rather than legality's always-live one — writing frontmatter markdown chunks (`document`, `section`, `title`, `source_url`, `version`, `fetched_at`) plus a rebuildable `kb/rules/index.json`. `kb/rules/` (chunks + index) is entirely git-ignored and rebuildable via `rules sync` — unlike `lore/`, which commits its OKF `.md` files and only ignores `index.json`/`.sync-state.json`; a re-sync supersedes each document's prior chunk set (no stale chunks) and per-source failures are isolated (one source failing doesn't block the others, including reprise vs. CR/TRP/PPG/CPG/legality).
- `fab-cli rules search <query>` / `fab-cli rules show <ref>` — query the KB above. `search` ranks chunks by simple term-overlap over `title`+`text` (mirrors `lore search`'s scoring) and auto-refreshes the whole KB first if `kb/rules/index.json` is missing or older than `FAB_RULES_TTL_MS` (default 7 days, env-overridable — mirrors `lore.ts`'s `FAB_LORE_TTL_MS`). `show <ref>` takes `<document>/<section>` (e.g. `cr/1.1`, case-insensitive) or a bare section slug (for chunks like CPG where the section text isn't a clean lookup key); an ambiguous or no-match ref prints candidates and exits 1 rather than guessing. **Card legality is always re-fetched live** whenever a `search`/`show` result includes the `legality` chunk — independently of the 7-day TTL above and on every single call (never cached/memoized across calls), per the same "legality is never served stale" rule as everywhere else in this CLI. When the KB is fresh and the query/ref doesn't touch legality, both commands run fully offline (no network) reading `kb/rules/index.json` from disk.
- `fab-cli rules ask "<question>"` — retrieves + cites the most relevant rules passages for a question (same citation format as `rules search`: document/section/source_url/snippet), reusing `searchRules()`'s TTL-refresh and legality-live guarantees unchanged. **It does not generate or paraphrase an answer** — no LLM synthesis of passage content, retrieval and citation only. Every single call ends with the judge Discord `#ask-a-judge` escalation footer (`https://discord.com/channels/874145774135558164/1020649907314495528`), unconditionally — there is no flag to suppress it. When the retrieved passages don't clearly settle the question (no passages matched at all, or fewer than half the query's terms hit the top-ranked passage), an extra highlighted line ("passages don't clearly settle this —") is printed immediately above the footer; the footer text/link itself never changes.
...
**Keyword corpus (HARD RULE):** the `kw-*` notes + generated `keywords-index` are ONE physical copy living in the card-vault brain, symlinked (relative) into the judge and player brains — all brains read the same bytes. Editorial authority is the judge's; notes follow a strict template; the index is generated; NEVER write through a `kw-*` symlink outside judge editorial. Verify/repair with `python3 fab-cli/scripts/keyword-sync.py check|sync` (run from the monorepo root — see below); full template + protocol: `.claude/identities/KEYWORD-SYNC.md`.

**Brain layout (monorepo, post-APP-001):** all identity brains — the FAB knowledge ones this section describes (judge, player, card-vault) AND the dev-loop ones (dev, reviewer, orchestrator, talishar, assistant) — live at the monorepo root's `.claude/identities/`, one level above this `fab-cli/` package, not under `fab-cli/.claude/`. The dev-loop brains stayed there deliberately: they serve the whole monorepo's build loop, not just this package. Because of this, `fab-cli/scripts/keyword-sync.py` and `fab-cli/scripts/backfill-entities.py` resolve `.claude/identities/` off the real git toplevel but `third_party/` off `fab-cli/third_party/` (that one *did* move under this package) — keep that split in mind if either script's paths ever need touching again.

**Cross-identity entities:** generated card/keyword notes declare stable `card:`/`keyword:` keys and builders refresh `.claude/identities/entity-index.json`. `python3 fab-cli/scripts/backfill-entities.py --check` previews conservative proposals for hand-owned judge/player notes; omit `--check` to write a reviewable diff (the script never stages or commits). Interaction notes should be minted with `--entities card:<a>,card:<b>` when those card files exist.
...
`.claude/talishar/dossiers/<card-slug>.md` (gitignored, ephemeral working state — never commit a
dossier) so a later phase or a resumed session can pick up the research without redoing it. If the
card isn't yet in the-fab-cube dataset (newly announced), the dossier records that gap instead of
fabricating stats and flags that `zzCardCodeGenerator.php` output needs regeneration once the
dataset submodule catches up — this is an expected, first-class outcome for brand-new cards, not
an error. Pure formatting/gap-detection/resume logic lives in `src/talisharDossier.ts`
(`slugifyCardName`, `detectDatasetGap`, `parseExistingStatus`, `shouldResumeDossier`,
...
- The official TCGplayer API is closed to new applicants — by design, `price-comparison` has no `auth` command; it relies entirely on the keyless tcgcsv mirror + storefront search endpoint.
- `tcgcsv.com` returns 401 without a browser-like `User-Agent` header — the client sets one by default; don't strip it when debugging.
- TCGplayer storefront search 403s are rate-limiting/bot-protection, not a crash — affected rows degrade to empty condition cells (never a fabricated stand-in price).
- Because TCGplayer condition prices come from live listings, two `export`/`card` runs of the same set/card minutes apart can show slightly different numbers; Cardmarket data is cached and stable within its 24h TTL.
