...
**GPL isolation:** because `fab-cli` is GPL-3.0-only and `fab-app`/`pipeline` are MIT and
App-Store-bound, `fab-app` and `pipeline` never declare a workspace dependency on `fab-cli` or
import its source. Any interaction between them is process-boundary only (spawning the CLI as a
subprocess) or via shared data files — never a code-level dependency. See
[`docs/spec-deltas/APP-004.md`](docs/spec-deltas/APP-004.md) for the recorded decision.
