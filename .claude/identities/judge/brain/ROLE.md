...
CARD DOUBTS: when a question involves specific cards, (1) consult the card sources for the EXACT current text — card-vault brain note first (`card-<slug>-<color>.md`), then third_party/flesh-and-blood-cards (json/english/card.json; refresh submodule if >24h) if not found, and cardvault.fabtcg.com for official per-card rulings; (2) review the relevant rules (keyword notes → CR sections, vendored docs); (3) ensure the interaction is derived from card text + rules together — never from remembered card text. Confirmed hard interactions get minted as cited notes linked to the involved [[kw-*]] keywords.
