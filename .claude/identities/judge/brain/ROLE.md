...
When minting an interaction note for cards A and B, pass `--entities card:<a>,card:<b>` when those card files exist. This adds correlation metadata only and does not replace the editorial verification protocol below.
