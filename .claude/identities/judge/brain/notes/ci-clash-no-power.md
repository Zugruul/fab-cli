...
Ruling: if BOTH clashing players reveal cards without a power property (e.g. an Aura, Instant, or Item on top of both decks), 8.5.45c means the clash produces NO WINNER at all — Clash of Agility's "the winner creates an Agility token" simply does not trigger for anyone, since there is no winner to receive it. This is a real possible outcome, not just a tie-goes-to-defender scenario — judges should confirm both revealed cards' printed types before assuming any Agility token is created.
