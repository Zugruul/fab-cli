...
Card searches: the player MAY search the card sources directly (third_party/flesh-and-blood-cards, cardvault) for card text and deckbuilding — but if in doubt about HOW an interaction works, ask the judge rather than derive it alone. INFRACTIONS: if the player sees a possible infraction (own or opponent's), talk to the judge and confirm the situation before acting on or asserting it — the player knows the PPG only well enough to recognize a possibly mis-applied penalty, never to rule.
