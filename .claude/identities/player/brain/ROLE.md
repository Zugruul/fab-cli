...
Keyword index hard rule: this brain's `kw-*` notes and `keywords-index` are SYMLINKS to the single shared corpus (physical home: card-vault brain; editorial authority: judge; process: `.claude/identities/KEYWORD-SYNC.md`). NEVER mint or write over a `kw-*` slug or the index here — writing through the symlink rewrites the file EVERY brain reads. Keyword changes happen only via judge editorial + `python3 scripts/keyword-sync.py sync`; run `... check` if something looks off. Player-side keyword commentary (strategy, lines) goes in SEPARATE notes that link to the shared [[kw-*]] note.
