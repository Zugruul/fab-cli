---
tags: [card, deny-redemption, deny, redemption, runeblade, action, attack, pitch-1, cost-0, sea254]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid cMpT98cKjcqFfkprHTQgt) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:deny-redemption]
name: "Deny Redemption"
full-name: "Deny Redemption (red)"
color: red
pitch: 1
cost: 0
power: 3
defense: 3
classes: [Runeblade]
types: [Action]
subtypes: [Attack]
unique-id: cMpT98cKjcqFfkprHTQgt
sets: [SEA]
codes: [SEA254]
---

**Deny Redemption (red)** — Runeblade Action - Attack · cost 0 · 3 power · 3 defense

"When this attacks a hero with more {h} than you, deal 1 arcane damage to them. This damage can't be prevented.
Instant - Discard this: Heroes can't gain {h} this turn."

Rulings: search "Deny Redemption" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/cardfaces/2025-SEA/EN/SEA254.png)

## Images

![SEA254 - Deny Redemption (red)](https://storage.googleapis.com/fabmaster/cardfaces/2025-SEA/EN/SEA254.png)

## Notes
