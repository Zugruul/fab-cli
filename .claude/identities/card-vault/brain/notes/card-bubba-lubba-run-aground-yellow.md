---
tags: [card, bubba-lubba-run-aground, bubba, lubba, run, aground, pirate, necromancer, action, ally, watery-grave, pitch-2, cost-3, pen157]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid QNjHwWjJFcDtq8KqpHRjP) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:bubba-lubba-run-aground]
name: "Bubba-Lubba, Run Aground"
full-name: "Bubba-Lubba, Run Aground (yellow)"
color: yellow
pitch: 2
cost: 3
power: 1
defense: 5
classes: [Pirate, Necromancer]
types: [Action]
subtypes: [Ally]
keywords: [Watery Grave]
unique-id: QNjHwWjJFcDtq8KqpHRjP
sets: [PEN]
codes: [PEN157]
---

**Bubba-Lubba, Run Aground (yellow)** — Pirate Necromancer Action - Ally · cost 3 · 1 power · 5 defense

"This enters the arena with a +1{p} counter.
Action - {r}, {t}: Attack
Action - Remove a +1{p} counter from an ally you control: Destroy an aura token. Go again
Watery Grave"

Keywords: [[kw-watery-grave]]
Rulings: search "Bubba-Lubba, Run Aground" at https://cardvault.fabtcg.com/

## Notes
