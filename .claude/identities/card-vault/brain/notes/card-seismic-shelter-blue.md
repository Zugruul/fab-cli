---
tags: [card, seismic-shelter, seismic, shelter, guardian, action, aura, go-again, pitch-3, cost-0, mpg023]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid wcmmpmGPLhHLFJfdWcpzm) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:seismic-shelter]
name: "Seismic Shelter"
full-name: "Seismic Shelter (blue)"
color: blue
pitch: 3
cost: 0
defense: 3
classes: [Guardian]
types: [Action]
subtypes: [Aura]
keywords: [Go again]
unique-id: wcmmpmGPLhHLFJfdWcpzm
sets: [MPG]
codes: [MPG023]
---

**Seismic Shelter (blue)** — Guardian Action - Aura · cost 0 · 3 defense

"Go again
Attack action cards you control get +X{d} while defending, where X is the number of Seismic Surge tokens you control.
At the start of your turn, destroy this."

Keywords: [[kw-go-again]]
Rulings: search "Seismic Shelter" at https://cardvault.fabtcg.com/
Images: [Image #1](https://d2wlb52bya4y8z.cloudfront.net/media/cards/large/MPG023.webp)

## Images

![MPG023 - Seismic Shelter (blue)](https://d2wlb52bya4y8z.cloudfront.net/media/cards/large/MPG023.webp)

## Notes
