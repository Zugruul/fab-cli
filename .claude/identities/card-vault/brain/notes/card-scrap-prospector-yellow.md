---
tags: [card, scrap-prospector, scrap, prospector, mechanologist, action, attack, pitch-2, cost-0, evo136]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid q68ThQwdMntNzPqwkwhC7) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:scrap-prospector]
name: "Scrap Prospector"
full-name: "Scrap Prospector (yellow)"
color: yellow
pitch: 2
cost: 0
power: 2
defense: 3
classes: [Mechanologist]
types: [Action]
subtypes: [Attack]
keywords: [Scrap]
unique-id: q68ThQwdMntNzPqwkwhC7
sets: [EVO]
codes: [EVO136]
---

**Scrap Prospector (yellow)** — Mechanologist Action - Attack · cost 0 · 2 power · 3 defense

"Scrap
When this attacks, if it scrapped a card, gain {r}."

Keywords: [[kw-scrap]]
Variants: [[card-scrap-prospector-red]] · [[card-scrap-prospector-blue]]
Rulings: search "Scrap Prospector" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/cardfaces/2023-EVO/EN/EVO136.png)

## Images

![EVO136 - Scrap Prospector (yellow)](https://storage.googleapis.com/fabmaster/cardfaces/2023-EVO/EN/EVO136.png)

## Notes
