---
tags: [card, reckless-stampede, reckless, stampede, brute, action, attack, clash, pitch-1, cost-2, sup127]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid bK9BLkb6M9JHT798FHgLP) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:reckless-stampede]
name: "Reckless Stampede"
full-name: "Reckless Stampede (red)"
color: red
pitch: 1
cost: 2
power: 7
defense: 0
classes: [Brute]
types: [Action]
subtypes: [Attack]
keywords: [Clash]
unique-id: bK9BLkb6M9JHT798FHgLP
sets: [SUP]
codes: [SUP127]
---

**Reckless Stampede (red)** — Brute Action - Attack · cost 2 · 7 power · 0 defense

"Whenever a card defends this, clash with the defending hero. The winner deals 1 damage to each other hero. Each hero may put their revealed card on the bottom of its owner's deck."

Keywords: [[kw-clash]]
Rulings: search "Reckless Stampede" at https://cardvault.fabtcg.com/

## Notes
