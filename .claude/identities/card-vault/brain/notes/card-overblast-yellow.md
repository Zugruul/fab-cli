---
tags: [card, overblast, mechanologist, action, attack, pitch-2, cost-2, cru113]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid MHkLpfgkgh7hNwbrTJrDF) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:overblast]
name: "Overblast"
full-name: "Overblast (yellow)"
color: yellow
pitch: 2
cost: 2
power: 4
defense: 3
classes: [Mechanologist]
types: [Action]
subtypes: [Attack]
unique-id: MHkLpfgkgh7hNwbrTJrDF
sets: [CRU]
codes: [CRU113]
---

**Overblast (yellow)** — Mechanologist Action - Attack · cost 2 · 4 power · 3 defense

"Overblast gains +X{p}, where X is the number of times you have boosted this combat chain."

Variants: [[card-overblast-red]] · [[card-overblast-blue]]
Rulings: search "Overblast" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/cardfaces/2020-CRU/CRU113.png) · [Image #2](https://storage.googleapis.com/fabmaster/cardfaces/2021-U-CRU/U-CRU113.png)

## Images

![CRU113 - Overblast (yellow)](https://storage.googleapis.com/fabmaster/cardfaces/2020-CRU/CRU113.png)
![CRU113 - Overblast (yellow)](https://storage.googleapis.com/fabmaster/cardfaces/2021-U-CRU/U-CRU113.png)

## Notes
