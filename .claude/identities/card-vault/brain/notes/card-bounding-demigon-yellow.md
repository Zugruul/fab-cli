---
tags: [card, bounding-demigon, bounding, demigon, runeblade, shadow, action, attack, blood-debt, pitch-2, cost-0, mon169]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid PRtnhQ68CgRRFJpwkpKbR) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:bounding-demigon]
name: "Bounding Demigon"
full-name: "Bounding Demigon (yellow)"
color: yellow
pitch: 2
cost: 0
power: 2
defense: 3
classes: [Runeblade]
talents: [Shadow]
types: [Action]
subtypes: [Attack]
keywords: [Blood Debt]
unique-id: PRtnhQ68CgRRFJpwkpKbR
sets: [MON]
codes: [MON169]
---

**Bounding Demigon (yellow)** — Shadow Runeblade Action - Attack · cost 0 · 2 power · 3 defense

"If you have played a 'non-attack' action card this turn, you may play Bounding Demigon from your banished zone. If you do, it gains +1{p}.
Blood Debt"

Keywords: [[kw-blood-debt]]
Variants: [[card-bounding-demigon-red]] · [[card-bounding-demigon-blue]]
Rulings: search "Bounding Demigon" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/media/images/MON169.width-450.png) · [Image #2](https://storage.googleapis.com/fabmaster/media/images/U-MON169.width-450.png)

## Images

![MON169 - Bounding Demigon (yellow)](https://storage.googleapis.com/fabmaster/media/images/MON169.width-450.png)
![MON169 - Bounding Demigon (yellow)](https://storage.googleapis.com/fabmaster/media/images/U-MON169.width-450.png)

## Notes
