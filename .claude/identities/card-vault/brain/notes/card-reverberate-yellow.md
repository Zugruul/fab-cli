---
tags: [card, reverberate, wizard, action, pitch-2, cost-1, 1hp327, arc139]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid nDWTKLbTdknK7Rfbp8mPM) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:reverberate]
name: "Reverberate"
full-name: "Reverberate (yellow)"
color: yellow
pitch: 2
cost: 1
defense: 3
classes: [Wizard]
types: [Action]
unique-id: nDWTKLbTdknK7Rfbp8mPM
sets: [1HP, ARC]
codes: [1HP327, ARC139]
---

**Reverberate (yellow)** — Wizard Action · cost 1 · 3 defense

"Deal 2 arcane damage to target opposing hero.
If Reverberate deals damage, you may banish a Wizard 'non-attack' action card from your hand with {r} cost less than or equal to the damage dealt by Reverberate. If you do, you may play it this turn as though it were an instant."

Variants: [[card-reverberate-red]] · [[card-reverberate-blue]]
Rulings: search "Reverberate" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/media/images/1HP327.width-450.png) · [Image #2](https://storage.googleapis.com/fabmaster/cardfaces/2020-ARC/ARC139.png) · [Image #3](https://storage.googleapis.com/fabmaster/cardfaces/2020-U-ARC/U-ARC139.png)

## Images

![1HP327 - Reverberate (yellow)](https://storage.googleapis.com/fabmaster/media/images/1HP327.width-450.png)
![ARC139 - Reverberate (yellow)](https://storage.googleapis.com/fabmaster/cardfaces/2020-ARC/ARC139.png)
![ARC139 - Reverberate (yellow)](https://storage.googleapis.com/fabmaster/cardfaces/2020-U-ARC/U-ARC139.png)

## Notes
