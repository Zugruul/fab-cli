---
tags: [card, consuming-aftermath, consuming, aftermath, shadow, action, attack, dominate, pitch-1, cost-3, lev023, mon195]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid GkqChmpPNNJKWdGDTnFnk) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:consuming-aftermath]
name: "Consuming Aftermath"
full-name: "Consuming Aftermath (red)"
color: red
pitch: 1
cost: 3
power: 6
defense: 2
talents: [Shadow]
types: [Action]
subtypes: [Attack]
keywords: [Dominate]
unique-id: GkqChmpPNNJKWdGDTnFnk
sets: [LEV, MON]
codes: [LEV023, MON195]
---

**Consuming Aftermath (red)** — Shadow Action - Attack · cost 3 · 6 power · 2 defense

"As an additional cost to play Consuming Aftermath, you may banish a card from your hand. If a Shadow card is banished this way, Consuming Aftermath gains dominate."

Keywords: [[kw-dominate]]
Variants: [[card-consuming-aftermath-yellow]] · [[card-consuming-aftermath-blue]]
Rulings: search "Consuming Aftermath" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/media/images/LEV023.width-450.png) · [Image #2](https://storage.googleapis.com/fabmaster/media/images/MON195.width-450.png) · [Image #3](https://storage.googleapis.com/fabmaster/media/images/U-MON195.width-450.png)

## Images

![LEV023 - Consuming Aftermath (red)](https://storage.googleapis.com/fabmaster/media/images/LEV023.width-450.png)
![MON195 - Consuming Aftermath (red)](https://storage.googleapis.com/fabmaster/media/images/MON195.width-450.png)
![MON195 - Consuming Aftermath (red)](https://storage.googleapis.com/fabmaster/media/images/U-MON195.width-450.png)

## Notes
