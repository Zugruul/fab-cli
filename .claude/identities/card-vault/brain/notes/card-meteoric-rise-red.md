---
tags: [card, meteoric-rise, meteoric, rise, lightning, action, attack, pitch-1, cost-1, fab472, omn153]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid cmbfFHBr6hjhrdmNNQGg6) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:meteoric-rise]
name: "Meteoric Rise"
full-name: "Meteoric Rise (red)"
color: red
pitch: 1
cost: 1
power: 5
defense: 2
talents: [Lightning]
types: [Action]
subtypes: [Attack]
unique-id: cmbfFHBr6hjhrdmNNQGg6
sets: [FAB, OMN]
codes: [FAB472, OMN153]
---

**Meteoric Rise (red)** — Lightning Action - Attack · cost 1 · 5 power · 2 defense

"Twice per Turn Instant - {r}: This gets +1{p}.
When this hits, create a Lightning Flow token."

Rulings: search "Meteoric Rise" at https://cardvault.fabtcg.com/
Images: [Image #1](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/FAB472-RF.webp) · [Image #2](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/OMN153.webp)

## Images

![FAB472 - Meteoric Rise (red)](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/FAB472-RF.webp)
![OMN153 - Meteoric Rise (red)](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/OMN153.webp)

## Notes
