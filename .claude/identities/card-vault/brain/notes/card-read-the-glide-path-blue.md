---
tags: [card, read-the-glide-path, read, glide, path, ranger, action, opt-1, go-again, pitch-3, cost-0, evr102]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid dN67MttzDLBTqmCWdDQ9M) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:read-the-glide-path]
name: "Read the Glide Path"
full-name: "Read the Glide Path (blue)"
color: blue
pitch: 3
cost: 0
defense: 2
classes: [Ranger]
types: [Action]
keywords: [Opt 1, Go again]
unique-id: dN67MttzDLBTqmCWdDQ9M
sets: [EVR]
codes: [EVR102]
---

**Read the Glide Path (blue)** — Ranger Action · cost 0 · 2 defense

"Your next arrow attack this turn gains +1{p}.
Opt 1
Go again"

Keywords: [[kw-opt]] · [[kw-go-again]]
Variants: [[card-read-the-glide-path-red]] · [[card-read-the-glide-path-yellow]]
Rulings: search "Read the Glide Path" at https://cardvault.fabtcg.com/
Images: [Image #1](https://storage.googleapis.com/fabmaster/media/images/EVR102.width-450.png)

## Images

![EVR102 - Read the Glide Path (blue)](https://storage.googleapis.com/fabmaster/media/images/EVR102.width-450.png)

## Notes
