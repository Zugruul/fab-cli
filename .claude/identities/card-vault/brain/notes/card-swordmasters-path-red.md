---
tags: [card, swordmasters-path, swordmasters, path, warrior, action, sharpen, go-again, pitch-1, cost-1, aha014]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid gTLKtttbcq6CcLMHghJKw) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:swordmasters-path]
name: "Swordmaster's Path"
full-name: "Swordmaster's Path (red)"
color: red
pitch: 1
cost: 1
defense: 3
classes: [Warrior]
types: [Action]
keywords: [Sharpen, Go again]
unique-id: gTLKtttbcq6CcLMHghJKw
sets: [AHA]
codes: [AHA014]
---

**Swordmaster's Path (red)** — Warrior Action · cost 1 · 3 defense

"Your next sword attack this turn gets +3{p}.
The next time you would sharpen a sword this turn, instead sharpen it an additional time.
Go again"

Keywords: [[kw-sharpen]] · [[kw-go-again]]
Variants: [[card-swordmasters-path-blue]]
Rulings: search "Swordmaster's Path" at https://cardvault.fabtcg.com/
Images: [Image #1](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/AHA014.webp)

## Images

![AHA014 - Swordmaster's Path (red)](https://legendstory-production-s3-public.s3.amazonaws.com/media/cards/large/AHA014.webp)

## Notes
