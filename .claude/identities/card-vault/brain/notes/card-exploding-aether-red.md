---
tags: [card, exploding-aether, exploding, aether, wizard, action, amp-3, go-again, pitch-1, cost-2, osc015, ros192]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid wPLfnbgJtbFDq7kgKjW6r) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:exploding-aether]
name: "Exploding Aether"
full-name: "Exploding Aether (red)"
color: red
pitch: 1
cost: 2
defense: 2
classes: [Wizard]
types: [Action]
keywords: [Amp 3, Go again]
unique-id: wPLfnbgJtbFDq7kgKjW6r
sets: [OSC, ROS]
codes: [OSC015, ROS192]
---

**Exploding Aether (red)** — Wizard Action · cost 2 · 2 defense

"Amp 3
Go again"

Keywords: [[kw-amp]] · [[kw-go-again]]
Variants: [[card-exploding-aether-yellow]] · [[card-exploding-aether-blue]]
Rulings: search "Exploding Aether" at https://cardvault.fabtcg.com/
Images: [Image #1](https://d2wlb52bya4y8z.cloudfront.net/media/cards/large/OSC015.webp) · [Image #2](https://storage.googleapis.com/fabmaster/cardfaces/2024-ROS/EN/ROS192.png)

## Images

![OSC015 - Exploding Aether (red)](https://d2wlb52bya4y8z.cloudfront.net/media/cards/large/OSC015.webp)
![ROS192 - Exploding Aether (red)](https://storage.googleapis.com/fabmaster/cardfaces/2024-ROS/EN/ROS192.png)

## Notes
