---
tags: [card, courageous-crossing, courageous, crossing, ranger, action, trap, go-again, pitch-3, cost-0, pen086]
paths: []
strength: 1
source: "third_party/flesh-and-blood-cards json/english/card.json (uid 7H6zRGbKbL8BpgNMgwh97) · https://cardvault.fabtcg.com/"
graduated: false
created: 2026-07-10
entities: [card:courageous-crossing]
name: "Courageous Crossing"
full-name: "Courageous Crossing (blue)"
color: blue
pitch: 3
cost: 0
defense: 3
classes: [Ranger]
types: [Action]
subtypes: [Trap]
keywords: [Go again]
unique-id: 7H6zRGbKbL8BpgNMgwh97
sets: [PEN]
codes: [PEN086]
---

**Courageous Crossing (blue)** — Ranger Action - Trap · cost 0 · 3 defense

"Create a Courage token under target hero's control.
Go again
When this defends an attack with {p} greater than its base, remove a +1{p} counter from target permanent."

Keywords: [[kw-go-again]]
Rulings: search "Courageous Crossing" at https://cardvault.fabtcg.com/

## Notes
