// BUG-190: behavior/cli.ts must read pipeline/out/chunks-fulltext.jsonl,
// not out/chunks.jsonl — see qa.cli.test.ts's doc comment for the full
// rationale (docs/rights-assessment.md §7.10's "Known follow-up gap").
// Note: behavior/build.ts's transforms (distractor/abstention/ood/dpo) only
// ever use a chunk's chunk_id/tags for categorization, never chunk.text
// (see categorize.ts, adjudication.ts) — so this lane can't structurally
// leak the stub marker into its output today either way. The reader
// plumbing is still repointed and still fails loudly on absence, for
// consistency with the other three lanes and as a defense-in-depth
// guarantee if that changes.
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { describe, it, expect, beforeEach, afterEach } from "vitest";
import { parseArgs, loadChunks } from "../src/behavior/cli.js";
...
describe("behavior/cli.ts — reads chunks-fulltext.jsonl, not chunks.jsonl (BUG-190)", () => {
  ...
  describe("loadChunks", () => {
    let tmpDir: string;
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fab-behavior-cli-fulltext-test-"));
    });
    afterEach(() => {
