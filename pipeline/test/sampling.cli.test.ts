// BUG-190: sampling/cli.ts must read pipeline/out/chunks-fulltext.jsonl,
...
import { parseArgs, loadChunksById } from "../src/sampling/cli.js";
...
describe("sampling/cli.ts — reads chunks-fulltext.jsonl, not chunks.jsonl (BUG-190)", () => {
  ...
  describe("loadChunksById", () => {
    let tmpDir: string;
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fab-sampling-cli-fulltext-test-"));
    });
    afterEach(() => {
    ...
    it("returns the real prose text from a fulltext-shaped file, exactly as written", () => {
      ...
      const map = loadChunksById(chunksPath);
      expect(map.get(LORE_CHUNK.chunk_id)).toEqual(LORE_CHUNK);
    });

    it("never returns the stub marker when pointed at the fulltext file (regression guard)", () => {
      const chunksPath = path.join(tmpDir, "chunks-fulltext.jsonl");
      fs.writeFileSync(chunksPath, JSON.stringify(LORE_CHUNK) + "\n");
      const chunk = loadChunksById(chunksPath).get(LORE_CHUNK.chunk_id)!;
    ...
    it("fails loudly with a re-run-export message when chunks-fulltext.jsonl is absent, rather than silently falling back to a sibling chunks.jsonl", () => {
      ...
      expect(() => loadChunksById(missingPath)).toThrow(/chunks-fulltext\.jsonl/);
      expect(() => loadChunksById(missingPath)).toThrow(/export/i);
    });
  });
});
