...
describe('benchmarkScreenReducer', () => {
  ...
  it('RUN_START moves to running and clears any previous result/error', () => {
    const errored = benchmarkScreenReducer(initialBenchmarkScreenState(), {
      type: 'RUN_ERROR',
      message: 'boom',
    });
  ...
  it('RUN_OK moves to done and attaches the result', () => {
    const running = benchmarkScreenReducer(initialBenchmarkScreenState(), {
      type: 'RUN_START',
    });
    const next = benchmarkScreenReducer(running, {
      type: 'RUN_OK',
      result: FAKE_RESULT,
    });
  ...
  it('RUN_ERROR moves to error and attaches the message, dropping any prior result', () => {
    ...
    const done = benchmarkScreenReducer(running, {
      type: 'RUN_OK',
      result: FAKE_RESULT,
    });
    const next = benchmarkScreenReducer(done, { type: 'RUN_START' });
    const errored = benchmarkScreenReducer(next, {
      type: 'RUN_ERROR',
      message: 'native module missing',
    });
    expect(errored).toEqual({
      status: 'error',
      errorMessage: 'native module missing',
    });
  });

  it('a re-run (RUN_START) from a done state clears the previous result while running', () => {
    const done = benchmarkScreenReducer(initialBenchmarkScreenState(), {
      type: 'RUN_OK',
      result: FAKE_RESULT,
    });
    const next = benchmarkScreenReducer(done, { type: 'RUN_START' });
