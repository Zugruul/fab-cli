...
const VALID_RUN_RESULT: BenchmarkRunResult = {
  ...
  metrics: {
    ...
    peakRamMb: { status: "not-run", reason: "no on-device RAM sampler wired yet" },
  },
};
...
describe("toManifestBenchmarkResult", () => {
  it("maps a valid run result + decision into a schema-valid manifest record, fields carried through verbatim", () => {
    ...
    expect(outcome.success, !outcome.success ? JSON.stringify(outcome.errors, null, 2) : undefined).toBe(true);
    if (outcome.success) {
      ...
      expect(outcome.manifestRecord.appVersion).toBe(VALID_RUN_RESULT.appVersion);
      expect(outcome.manifestRecord.buildNumber).toBe(VALID_RUN_RESULT.buildNumber);
      expect(outcome.manifestRecord.iterations).toBe(VALID_RUN_RESULT.iterations);
      expect(outcome.manifestRecord.startedAt).toBe(VALID_RUN_RESULT.startedAt);
      expect(outcome.manifestRecord.completedAt).toBe(VALID_RUN_RESULT.completedAt);
  ...
  it("returns success: false with schema errors for an invalid combination (completedAt before startedAt), never throwing", () => {
    ...
    const decision: FallbackLadderDecision = { targetsMet: true, unmetMetrics: [], notes: "x" };
    const outcome = toManifestBenchmarkResult(badRunResult, decision);
    expect(outcome.success).toBe(false);
    if (!outcome.success) {
      expect(outcome.errors.length).toBeGreaterThan(0);
      expect(outcome.errors.some((e) => e.path.join(".") === "completedAt")).toBe(true);
    }
  });
