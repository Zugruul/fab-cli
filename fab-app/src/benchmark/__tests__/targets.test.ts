...
import { metricMeetsTarget, checkTargets, TARGETS_BY_TIER, RETRIEVAL_P95_TARGET_MS } from "../targets";
...
describe("metricMeetsTarget boundary behavior", () => {
  it("'lt' target: exactly at threshold FAILS (strict less-than)", () => {
    expect(metricMeetsTarget(3000, { compare: "lt", threshold: 3000 })).toBe(false);
  });

  it("'lt' target: just under threshold PASSES", () => {
    expect(metricMeetsTarget(2999.99, { compare: "lt", threshold: 3000 })).toBe(true);
  });

  it("'lt' target: just over threshold FAILS", () => {
    expect(metricMeetsTarget(3000.01, { compare: "lt", threshold: 3000 })).toBe(false);
  });
  ...
  it("'gte' target: just under threshold FAILS", () => {
    expect(metricMeetsTarget(9.99, { compare: "gte", threshold: 10 })).toBe(false);
  });

  it("'gte' target: just over threshold PASSES", () => {
    expect(metricMeetsTarget(10.01, { compare: "gte", threshold: 10 })).toBe(true);
  });
});
...
describe("checkTargets", () => {
  ...
  it("excludes prefillTokensPerSec from gating (§14 has no independent target row for it)", () => {
    const withAbsurdPrefill: BenchmarkMetrics = { ...ALL_PASS_1_7B, prefillTokensPerSec: measured(0.001) };
    expect(checkTargets(withAbsurdPrefill, "1.7B").allMet).toBe(true);
    expect(checkTargets(withAbsurdPrefill, "1.7B").checks.some((c) => c.metric === "prefillTokensPerSec")).toBe(
      false,
    );
  });

  it("a not-run metric counts as unmet — an unmeasured metric can never be confirmed to meet its target", () => {
    const withNotRunRam: BenchmarkMetrics = { ...ALL_PASS_1_7B, peakRamMb: notRun("no RAM sampler wired") };
    const result = checkTargets(withNotRunRam, "1.7B");
    expect(result.allMet).toBe(false);
    expect(result.unmetMetrics).toContain("peakRamMb");
    expect(result.checks.find((c) => c.metric === "peakRamMb")?.status).toBe("not-run");
  });

  it("a measured metric that fails its target is reported unmet", () => {
    const withSlowTtft: BenchmarkMetrics = { ...ALL_PASS_1_7B, ttftWarmMs: measured(3000) }; // exactly at bound, strict lt fails
  ...
  it("the same metrics can pass the 1.7B tier but fail the stricter 0.6B decode floor", () => {
    const decode12: BenchmarkMetrics = { ...ALL_PASS_1_7B, decodeTokensPerSec: measured(12) };
  ...
  it("gates retrievalP95Ms against the fixed APP-033 50ms bound regardless of tier", () => {
    const slowRetrieval: BenchmarkMetrics = { ...ALL_PASS_1_7B, retrievalP95Ms: measured(50) }; // strict lt fails at exactly 50
    expect(checkTargets(slowRetrieval, "1.7B").unmetMetrics).toContain("retrievalP95Ms");
    expect(checkTargets(slowRetrieval, "0.6B").unmetMetrics).toContain("retrievalP95Ms");
  });
});
