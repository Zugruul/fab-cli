...
import { HashingBagOfWordsEmbedder, InMemoryChunkCorpus, StaticVectorStore } from "../../retrieval/__tests__/testDoubles";

const BASE_CONFIG: Pick<BenchmarkRunnerConfig, "tier" | "device" | "appVersion" | "buildNumber" | "iterations"> = {
  tier: "1.7B",
...
describe("BenchmarkRunner", () => {
  it("rejects a non-positive iterations count at construction", () => {
    expect(() => new BenchmarkRunner({ ...BASE_CONFIG, iterations: { iterations: 0, warmupIterations: 0 } })).toThrow();
    expect(
      () => new BenchmarkRunner({ ...BASE_CONFIG, iterations: { iterations: -1, warmupIterations: 0 } }),
    ).toThrow();
  });

  it("rejects a negative warmupIterations count at construction", () => {
    expect(
      () => new BenchmarkRunner({ ...BASE_CONFIG, iterations: { iterations: 3, warmupIterations: -1 } }),
    ).toThrow();
  });

  it("reports every metric as not-run, with a distinct reason each, when no clients are injected", async () => {
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, now: fixedClock(["2026-08-01T00:00:00.000Z"]) });
    const result = await runner.run();
    for (const key of Object.keys(result.metrics) as (keyof typeof result.metrics)[]) {
      expect(result.metrics[key].status, key).toBe("not-run");
      if (result.metrics[key].status === "not-run") {
        expect((result.metrics[key] as { reason: string }).reason.length).toBeGreaterThan(0);
      }
    }
  });
  ...
  it("carries top-level fields through exactly (tier, device, appVersion, buildNumber, iterations, timestamps)", async () => {
    ...
    expect(result.device).toEqual({ model: "iPhone 13 Pro", osVersion: "iOS 17.5.1" });
  ...
  it("computes decode tok/s as the MEDIAN (p50) of per-iteration rate, discarding warmup", async () => {
    ...
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, decode: { client, tokenCount: 64 } });
    const result = await runner.run();
    expect(result.metrics.decodeTokensPerSec).toEqual({ status: "measured", value: 1000 });
  ...
  it("computes prefill tok/s as the MEDIAN (p50) of per-iteration rate, discarding warmup", async () => {
    ...
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, prefill: { client, promptTokenCount: 1024 } });
    const result = await runner.run();
    expect(result.metrics.prefillTokensPerSec).toEqual({ status: "measured", value: 2048 });
  });
  ...
  it("computes TTFT warm and cold independently as p95, each with its own injected client", async () => {
    ...
    const result = await runner.run();
    expect(result.metrics.ttftWarmMs).toEqual({ status: "measured", value: 3000 });
    expect(result.metrics.ttftColdMs).toEqual({ status: "measured", value: 7000 });
  });
  ...
  it("computes query-embedding latency as p95, cycling through the supplied queries", async () => {
    ...
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, embedding: { client, queries: ["dominate", "go again"] } });
    const result = await runner.run();
    expect(result.metrics.queryEmbeddingLatencyMs).toEqual({ status: "measured", value: 300 });
  });
  ...
  it("reports embedding as not-run when queries is empty even though a client is present", async () => {
    ...
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, embedding: { client, queries: [] } });
    const result = await runner.run();
  ...
  it("computes RAM peak in MB as the max sample across sampleCount calls", async () => {
    const sampler = new FakeRamPeakSampler([1 * 1024 * 1024, 5 * 1024 * 1024, 2 * 1024 * 1024, 3 * 1024 * 1024]);
    const runner = new BenchmarkRunner({ ...BASE_CONFIG, ramPeak: { sampler, sampleCount: 4 } });
    const result = await runner.run();
  ...
  it("measures retrieval p95 against the REAL RetrievalEngine (APP-033), not a fake interface", async () => {
    ...
    const corpus = new InMemoryChunkCorpus([
      { id: "c1", text: "dominate ability text", tags: ["dominate"], cardNames: [], links: [], source: { document: "test", url: "https://example.test/c1" } },
      { id: "c2", text: "go again reaction text", tags: ["go-again"], cardNames: [], links: [], source: { document: "test", url: "https://example.test/c2" } },
    ]);
    const vectorStore = new StaticVectorStore([{ chunkId: "c1", score: 0.9 }]);
    const engine = new RetrievalEngine(corpus, vectorStore, embedder, { retrievalFloor: 0.3, oodThreshold: 0.1 });
    const runner = new BenchmarkRunner({
      ...BASE_CONFIG,
